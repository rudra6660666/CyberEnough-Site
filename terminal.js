...
class KaliTerminal {
    constructor() {
        ...
        // Simulated directories for cd/ls
        this.directories = {
            '~': ['Desktop', 'Documents', 'Downloads', 'Music', 'Pictures', 'Videos', 'tools', 'wordlists', 'exploits', 'scripts'],
            'Desktop': [],
            'Documents': [],
            'Downloads': [],
            'Music': [],
    ...
    // Add/changeDirectory method for cd command
    changeDirectory(args) {
        if (args.length === 1 || args[1] === '~') {
            this.currentPath = '~';
        } else {
            const target = args[1];
            // Allow cd .. to go back to ~ only
            if (target === '..') {
                this.currentPath = '~';
            } else if (this.directories[this.currentPath] && this.directories[this.currentPath].includes(target)) {
                this.currentPath = target;
            } else if (this.directories[target]) {
                this.currentPath = target;
            } else {
                this.addOutput(`bash: cd: ${target}: No such file or directory`, 'terminal-error');
            }
        }
    }

    // Update listDirectory to use simulated directories
    listDirectory(args) {
        const files = [
            '.bashrc', '.profile'
        ];
        const dirs = this.directories[this.currentPath] || [];
        const all = [...dirs, ...files];

        if (args.includes('-la') || args.includes('-l')) {
            const detailed = all.map(file => {
                const isDir = dirs.includes(file);
            ...
            this.addOutput(all.join('  '), 'terminal-output-text');
        }
    }

    // Update printWorkingDirectory to reflect currentPath
    printWorkingDirectory() {
        if (this.currentPath === '~') {
            this.addOutput(`/home/${this.username}`, 'terminal-output-text');
        } else {
            this.addOutput(`/home/${this.username}/${this.currentPath}`, 'terminal-output-text');
        }
    }
